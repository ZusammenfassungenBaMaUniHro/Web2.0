...
\section{Definitionsversuche}
	...
	\end{center}
	\begin{itemize}
		\item Charakterisierung möglich durch Auswahl verschiedener Waaben als Baublöcke
	...
	\begin{description}
		...
		Finden, was das Unternehmen weiß und in welchen Köpfen sich das Wissen befindet
		\item[LINKS] Ähnliches verbinden nach Assoziationen und Gemeinsamkeiten;
		Abkehr vom  \glqq Teile und herrsche\grqq 
		...
		\item[EXTENSIONS] Erweiterung der eigenen Sicht und Bewertung (Empfehlungssysteme, Bewertungsportale)
		\item[SIGNALS] Push und Pull durch signalbasierte Kanäle ersetzen
	\end{description}
	...
	\subsection{Konzepte}
	\begin{itemize}
		\item Entwicklung des Web 2.0 führte zu einer Reihe von neuen Ansätzen
		\begin{description}
			...
			\item[Social Media:] Facebook, etc.
		\end{description}
	...
	Der Inhalt wird durch den Nutzer erstellt. Mögliche Kategorisierungen von Web 2.0 Anwendungen:
		\begin{description}
			\item [Upload Content:] Der Benutzer lädt den Inhalt einer Webseite hoch (Youtube, Flickr,...)
			\item [Complete User Dominance:] Der Benutzer ist zentrales Element (Foren, Blogs, Wikis, Twitter,...)
			\item [Trust Themen:] Wie vertrauenswürdig ist der erstellte Content? 
			\item [Kollaboration:] Die Benutzer arbeiten zusammen (Google Docs, Etherpad,...)
		\end{description}
	\subsubsection{Anwendungen - Cooperation Portals}
	 Mögliche Kategorisierung von Coopartion Portalen:
		\begin{description}
			\item[Business Networks:] Plattform für den Austausch von unternehmesrelevanten Informationen\\
			(XING; LinkedIn,...)
			\item[Interest Networks:] Plattform für den Austausch von Interessen der Benutzer (Facebook, MySpace,...)
			\item[Synchronisation:] Plattform für die Synchronisation von Benutzerinformationen über verschiedene Endgeräte\\
			(Foxmarks(Lesezeichen), Plaxo(Kontaktdaten),...)
		\end{description}
	\subsubsection{Technologien - Konzepte}
	\begin{description}
		\item[Advanced JavaScript:] Einsatz von komplexen JavaScript Programmen für die Realisierung der Webseite
		\item[AJAX, XHR, HTML5:]  Realisierung einer dynamischen Webseite durch asynchrone Kommunikation
		\item[Folkonomies und Tags:] Webseite stellt eine Benutzer definierte Möglichkeit für die Organisation der Daten\\
		(Hashtag, Subreddit,...)
		\item[Browser Extensions und Plugins:] Erweiterung der Browser durch den Anwendungsentwickler, dadurch Desktop Look-and-Feel
		\item[Probabilistische Algortihmen (Bitcoin):] Einsatz von Probablistischen Algorithmen auf verteilter Basis für die Erbrining einer Leistung (Bitcoin - Online Wallet)
	\end{description}	
	\subsubsection{Technologien - Werkzeuge}
	\begin{description}
		\item[JS Libraries:] jQuery, extJS, Angular
		\item[Embedded JS:] Einsatz von JavaScript auf Server/Desktop Ebene (V8, node.js,...)
		\item[Ajax KITS:] Implementierung für die asynchrone Kommunikation (Rico, Dojo, GWT,...)
	\end{description}
	\subsection{Notizen}
		\begin{itemize}
			\item Silos and Boundaries
				...
				\item Die Daten-Sammel-Phase ist zuende $\rightarrow$ wie sollen sie verarbeitet werden? $\rightarrow$ Google weiß bspw. schon vorher, ob eine Grippewelle eintritt (anhand der Häufigkeit von Suchanfragen)
		\end{itemize}
...
abhängig von der Zahl an Teilnehmern am Gesamtsystem
		...
		\begin{itemize}
			\item Nebeneffekt der Tatsache, dass ein Netzwerk
...
Netzwerk-Effekte mehr Nutzen als ein kleineres, besseres System)
			...
			\item Pinguin Effekt
				\begin{itemize}
					...
					\item \textbf{Negativer Fall}: Idee geht pleite bevor sie Nutzer hat
...
\glqq Reclaim your Inbox\grqq
	...
	\begin{itemize}
		...
		\item An \glqq Träger\grqq gebunden durch:
		\begin{itemize}
			\item Name
			\item Kenntnis
			\item Pseudonym
			\item Den Besitz eines Objekts
			\item Körper
		\end{itemize}
	...
	\subsection{Diskussion}
			...
			\item Pro
				\begin{itemize}
				 ...
				 \item Absolutes \glqq Bank\grqq geheimnis möglich, da keine Bank
	...
	\begin{itemize}
		...
		\begin{itemize}
			\item den Surfer eine Webseite des Angreifers laden lassen
			\begin{enumerate}
		...
		\item Konzentation auf Cross Site Angriffe (XHR, CSS, Scripting, Request Forgery)
	...
	\begin{itemize}
		...
		\begin{itemize}
			...
			\item Redirections
		\end{itemize}
		\item Durchbrechen der Sandbox \textbf{benutzerkontrolliert} oder \textbf{systematisch} nach definierten Standard (HTML5 File Reader/Writer)
		\item Schutzziele:
		\begin{itemize}
	...
	\begin{itemize}
		...
		\item \textit{message} = String oder Object das gesendet wird
		\item \textit{targetURL} = URL des Zielfensters
	...
	\begin{itemize}
		\item Eine Komponente hat ein Recht, aber das Recht wird außerhalb der erwarteten Verarbeitungssequenz/-kontexts genutzt
		\item Eine Komponente referenziert eine andere Komponente, die aber anders als mit der intendierten Semantik der Komponente genutzt wird 
	\end{itemize}
	...
	\begin{itemize}
		...
		\begin{itemize}
			...
			\item Programm hat unter einer UserId bestimmte Zugriffsrechte
			\item \textbf{Problem}: Kann Zugriffsrechte nicht mit anderen Nutzern teilen
	...
	\begin{itemize}
		...
		\item Strokejacking: Stiehl den Tastendruck des Users (Browser sieht andere Taste als die Vom Nutzer gedrückte)
	\end{itemize}
	...
	\begin{itemize}
		...
		\item Später neue Anforderungen: Zugriff auf Hardware \& Sensoren (GPS, Webcam, Mic), Performance Boost durch clientseitige Parallelität und stärkere Kontrolle der User (App Stores, Binary Only Module)
		\item JavaScript kann: Zugriff auf Webseite+Header, Event Handler, Schnittstelle Java - JavaScript
		\item Vorteile von JavaScript:
		\begin{itemize}
			\item Extrem weite Verbreitung
	...
	\begin{itemize}
		\item Zugriff auf Parameter über Namen oder Array-Argumente (variadische Funktionen)
...
20 + 5 + "px" 		// ist nicht so ganz klar !
		...
		\begin{itemize}
			\item Ohne das Erzwingen eines \glqq new\grqq bei der Objekterzeugung kann es zu Fehlverhalten kommen
			 \begin{lstlisting}
...
var Person = function (name) {this.name = name;}.
	...
	\begin{itemize}
		...
		\item \textit{open()}: öffnet Dokument zum Neubeschreiben
		\item \textit{close()}: schließt Dokument
		\item \textit{write()}: schreibt Text in das Dokument
		\item \textit{writeln()}: schreibt Zeile in das Dokument
		\item Anwendung:
		\begin{itemize}
			...
			\item \textit{open-write-close} Zyklus: schreibt in ein anderes Fenster aus einem fremden Dokument heraus (close nötig, da sonst Browser das Dokument nicht lädt)
		\end{itemize}
...
--23xx1211--
	...
	\begin{itemize}
		...
		\begin{itemize}
			\item \textit{createElement}: erzeugt einen Knoten
			\item \textit{setAttribute}: setzt einen Attributwert (Zugriff meistens auch über JavaScript Property möglich)
			\item \textit{setNodeValue}: setzt den Textwert eines Knoten
			\item \textit{appendChild}: fügt einen Knoten an eine Liste von Kindern
			\item \textit{removeChild}: entfernt ein Kind
			\item \textit{replaceChild}: ersetzt ein Kind
		\end{itemize}
	...
	\begin{itemize}
		...
		\begin{itemize}
			...
			\item Slow load liefert keine Daten oder Daten mit \glqq NOP\grqq -Semantik, bis der Server dann wirklich etwas vom Client will
		\end{itemize}
		\item Realisierungen:
		\begin{itemize}
...
\section{Serverseitige Performance}
	\begin{itemize}
		...
		\begin{itemize}
			...
			\item Problem wird schwerer, da durch Web 2.0 das Web stärker fragmentiert wird(Mehr Ressourcen (Bilder, CSS, JS), mehr Kanäle (Websockets), häufigere Interaktion (Responsive Web))
		\end{itemize}
...
\section{Privatheit}
	\begin{itemize}
		...
		\item \textbf{Digitales Panopticon\footnote{Panopticon: rundes Gefängnis auf Kuba}}: Datensammlungen führen allein durch ihr Bestehen bereits zu einer Verhaltensänderung; Demonstriert durch Plakate, die Augen darstellen und so den Diebstahl von Fahrrädern reduzieren.
	\end{itemize}
	...
	\begin{itemize}
		\item Menschenrechtsorganisation mit Sitz in London - beobachtet weltweite Entwicklung der Privatheit 	\\
		$\rightarrow$ Ausmaß von Privacy Verletzungen und Rückgang von privacy ist erschreckend
	\end{itemize}
	...
	\end{itemize}
			...
			\begin{itemize} 
				...
				\end{itemize}
				\item Begriffe: \textbf{klassisches Pseudonym} (Nickname aber nicht die bürgerliche Identität bekannt) \& \textbf{Linkable Pseudonym} (Nickname und bürg. Name sind unbekannt, es ist nur bekannt, dass die Person bspw. 2 Posts geschrieben hat) \& \textbf{Anonymität}
				...
				\begin{itemize}
					...
					\end{itemize}
					\item \textbf{Timing Analyse} bei kleiner Anonymitäts-Menge und geringem Traffic führt bei geringer Latenz-Zeit zu rascher Enttarnung des Benutzers durch einen Angreifer mit Fähigkeit zur (globalen) Verkehrsanalyse.
					\item Bequemlichkeit Problem:
					\begin{itemize}
						\item Anonymitäts-Software macht Aufwand
						\item Wird daher nur für \glqq heikle\grqq Inhalte genutzt
...
\section{Attention Economy}
	\begin{itemize}
		\item Agrargesellschaft $\rightarrow$ Industriegesellschaft $\rightarrow$ Wissensgesellschaft (Wie komme ich möglichst schnell an möglichst viel Information?)
...
Roman)
		...
		\item Community
		\begin{itemize}
			\item Vermittelt ein Gefühl des \glqq Ownerships\grqq
	...
	\begin{enumerate}
		...
		\begin{itemize}
			...
			\item \textbf{Filtern}: Verhindern, dass unerwünschte Inhalte meine Aufmerksamkeit beanspruchen - Bsp: Ad Blocker, Spam Filter
		...
		\begin{itemize}
			...
			\begin{itemize}
				\item \textbf{Fremdbestimmt}, ich habe wenig Einfluss auf den Mechanismus (Google: Pagerank bestimmt)
...
\section{Ruby on Rails}
	\begin{itemize}
		...
		\item Bietet Möglichkeiten zur automatischen Erstellung der Datenbankstruktur auf Basis des Models (Scaffolding)
	\end{itemize}
...
\section{Clientseitige Performance}
	...
	\begin{itemize}
		\item Da nicht compiliert wird, muss der Entwickler per Hand Optimieren
	...
	\begin{itemize}
		\item Problem: mehrfacher Zugriff auf Variablen im selben Kontext ist langsam, da jedes Mal eine Kette von Pointern abgearbeitet werden muss
	...
	\begin{itemize}
		\item Client-Side Caching: Vielzahl verfügbarer Header für Caching Einstellung am Client, Ansteuerung durch Selbsterstellung des Headers oder Server-abhängige Konfiguration
...
\section{HTML 5}
	...
	\begin{itemize}
		...
		\begin{itemize}
			\item \textit{section} = Abschnitt eines Dokumentes welches explizit im Outline/TOC erscheinen soll (Kind mit h1 bis h6)
			\item \textit{article} = Vollständig in sich abgeschlossener Artikel, der auch separat genutzt werden kann (Blog-Eintrag, User-Kommentar)
			\item \textit{aside} = Zusatzinformation mit Bezug zum sonstigen Inhalt, die aber getrennt vom sonstigen Inhalt aufscheinen kann (Sidebar)
			\item Definitionen sind sinnvoll aber willkürlich $\rightarrow$ muss man sehen wie es aufgegriffen wird
		\end{itemize}
		\item Untergliedernde Elemente (Teil einer Section)
		\begin{itemize}
			\item \textit{header} = Enthält h1-h6 aber kann auch Lead-Absatz, Abstract enthalten
			\item \textit{footer} = zusätzliche ergänzende Information (Copyright, Autor, Meta)
			\item \textit{nav} = bündelt Links und navigatorische Elemente
		\end{itemize}
		\item Gruppierende Elemente
		\begin{itemize}
			\item \textit{details} = Zusatzinformation, die User nach Wunsch ein- und ausblenden kann
			\item \textit{figure} = Gruppierungselement, das auch außerhalb des normalen Flows dargestellt werden darf (wie in Latex floats)
			\item \textit{figurecaption} = Teil von Figure; Bildbeschreibung
		\end{itemize}
		\item Neue Input Typen
		\begin{itemize}
			\item \textit{color} = Colorpicker
			\item \textit{date} = Kalender zur Datumsauswahl
			\item \textit{datetime} = data + Uhrzeit
...
\begin{itemize}
	...
	\item \textbf{Dokument reinigen} | Jede Art von Meta-Daten des Dokuments oder im Dokument entfernen, die (unerwünschte zutreffende) Rückschlüsse auf den Leaker geben (z.B. Seriennummern, Versionsnummern, Beschnittene Bildteile, Wasserzeichen etc.)
...
Plattformen
\begin{itemize}
	\item \textbf{Cryptome.org} | Durch New Yorker Architekt John Young ins Leben gerufen hat die Leaking-Plattform mit dem Motto \glqq Expect to be deceived\grqq seit 1996 12 GB an Material geleaked. Dokumente werden ungeprüft \glqq as is\grqq eingestellt. Am Tag kommen 4 - 6 neue Dateien hinzu.
...
\section{Projekt}
Mögliche Fragethemen:
	\begin{itemize}
		\item Doku
		\item was gelernt?	
		\item Wirtschaft/Akzeptanz
		\item wird ankommen
		\item warum nutzen
		\item risiken
		\item marketing
